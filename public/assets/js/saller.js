...
//   }
  ...
  const routes = {
    getClients: 'sallers/data',
    addClient: 'sallers/add-client',
    updateClient: 'sallers/update-client',
    deleteClient: 'sallers/delete-client'
  };
  ...
  const db = {
    ...
    insertItem(item) {
      ...
      for (const key in item) {
        if (key === "logo" && item.logo instanceof File) {
          formData.append("logo", item.logo);
        } else {
            plainData[key]=item[key];
        //   formData.append(key, item[key]);
        }
      }
      const encryptedPayload=encryptDataJS(JSON.stringify(plainData));
...
formData.append('payload',encryptedPayload);
  ...
  $.get(routes.getClients, function (data) {
      ...
      fields: [
        {
          ...
          itemTemplate: function (_, item) {
            const grid = $("#js-saller").data("JSGrid");
    ...
    $("#js-saller .jsgrid-filter-row input, #js-grid .jsgrid-filter-row select").on("input change", function () {
      $("#js-saller").jsGrid("loadData");
    });
  });
})(jQuery);
